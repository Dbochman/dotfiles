...
| `financial-scrape-0001` | Sundays 4:05 AM ET | `exec` | `none` (agent self-messages on failure only) | Weekly financial dashboard refresh: Tesla Solar (API), Eversource / NG Electric / BWSC (Tier 2 self-healing via Pinchtab cookie re-auth), NG Gas / BoA / PennyMac mortgages (Tier 3 headless, alert on failure), then SQLite imports. Production Plaid sync is a separate daily cache-only LaunchAgent. Re-add procedure: see `FINANCIAL-DASHBOARD.md` and `LAUNCHAGENTS.md`. |
