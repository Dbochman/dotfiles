...
Most service logs live at `~/.openclaw/logs/` on the Mac Mini (and MacBook Pro for Crosstown presence). The current generated gateway service is the exception: it writes to `~/Library/Logs/openclaw/gateway.log`. Both locations survive reboots; older `/tmp/` logs did not.
...
| **Generated gateway service** | OpenClaw's service environment wrapper launches the FDA app wrapper | `~/Library/Logs/openclaw/gateway.log` |
...
- `group-sync*` and `sync-imessage-groups*` — legacy group sync retired with BlueBubbles; native group routing uses `channels.imessage.groups`
