...
TRIAGE_SPEC = importlib.util.spec_from_file_location(
    "julia_triage_common", Path(__file__).with_name("morning-triage.py")
)
...
HANDOFF_COUNTERS = triage.HANDOFF_COUNTERS
...
validate_handoff = triage.validate_handoff
review_presentation = triage.review_presentation
validate_handoff_file = triage.validate_handoff_file
...
def load_triage_handoff(
    ...
    *,
    ...
    return triage.load_triage_handoff(
        today, db_path=db_path, job_id=TRIAGE_JOB_ID, store_key=CRON_STORE_KEY,
    )
...
def main(argv=None) -> int:
    ...
    if args.validate_handoff:
        try:
            payload = validate_handoff_file(args.validate_handoff)
