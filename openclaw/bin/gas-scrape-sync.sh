#!/bin/bash
# gas-scrape-sync.sh — Scrape National Grid portal and sync gas data to Mac Mini
#
...
#
# Usage:
#   gas-scrape-sync.sh              # Full scrape + sync
#   gas-scrape-sync.sh --dry-run    # Scrape only, don't sync to Mini
...
LAST_RUN_FILE="${REPO_DIR}/.gas-scrape-last-run"
...
# --- Step 1: Scrape National Grid portal ---
log "Scraping National Grid portal..."
if ! python3 scrape_national_grid.py --headless --merge; then
...
if ! python3 update_data.py import-json-gas; then
...
log "Done. Gas data synced successfully."
