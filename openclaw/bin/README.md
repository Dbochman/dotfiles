...
| `home-event-camera.py` | Separate bounded +30/+60 evidence worker for fresh confirmed-vacant canary events; rechecks canonical vacancy before each slot and cancels safely after an arrival or uncertain state, otherwise combines the exact triggering Ring camera (or site front door) with the exact per-site Nest interior camera, retains only a strict aggregate person-visible result plus provider-specific failure codes, immediately deletes every frame, and cannot change alert eligibility. |
