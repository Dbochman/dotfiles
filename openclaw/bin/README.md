...
| `home_event_action.py` | Exact policy-owned Hue and Petlibro worker. Feeder transfer actions require fresh paired Whisker coverage plus canonical vacancy/occupancy, restore an OpenClaw-owned destination pause before suspending the origin, verify every schedule state, and never adopt or resume a manual pause. |
