...
| `julia-morning-briefing-data.py` | repo path (cron invokes it directly) | Collects Julia's validated same-day triage handoff, current Calendar, cached sleep, aggregate finances, and bounded post-triage unread metadata. Only a successful triage establishes the new-arrival baseline; failed/partial triage never suppresses healthy independent sections. Owns raw GWS account routing, pagination, retries, deadlines, and section-level failure handling. |
