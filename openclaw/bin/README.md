...
| `weekly-financial-scrape.py` | — | Sunday 04:05 deterministic cache-only scraper orchestrator. Never reads `.env-token` or invokes `op`; validates five profiles in its dedicated owner-only JSON cache, strips stale finance names from ordinary children, and scopes one pair only to a guarded re-auth child. BWSC keeps its normal HTTP-first command, confirms uncertain auth once in Playwright, and releases its pair only after the fixed authentication-required marker; non-auth safe failures never import. It discards BWSC child output except for one allowlisted `BWSC_PATH` mode, making persistent browser recovery visible without retaining financial data. The helper serializes whole runs with a protected lock, starts or reuses BoA's dedicated headless `finance` PinchTab profile, binds every BoA child to that exact acquired instance ID, privately opens the single fixed secure-host BoA sign-in URL only after an exact missing-tab or public signed-out-landing probe, re-verifies before selecting credentials, contains timed-out children in private process groups, captures child output privately, preserves safe raw-CDP recovery statuses, imports only current-run successes, and enforces mortgage run-ID freshness. `--preflight` performs no browser or data mutation. |
