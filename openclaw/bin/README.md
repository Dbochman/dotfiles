...
| `home-event-camera.py` | Separate bounded +30/+60 evidence worker for fresh confirmed-vacant canary events; captures exact per-site Nest aliases, retains only a strict person-visible result, immediately deletes each frame, and cannot change alert eligibility. |
