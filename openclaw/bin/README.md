...
| `home-event-camera.py` | Separate bounded +30/+60 evidence worker for fresh confirmed-vacant canary events; combines the exact triggering Ring camera (or site front door) with the exact per-site Nest interior camera, retains only a strict aggregate person-visible result plus provider-specific failure codes, immediately deletes every frame, and cannot change alert eligibility. |
