...
qmd context add qmd://workspace/ "OpenClaw runtime workspace: SOUL.md (agent personality/rules), TOOLS.md (reservation routing, native iMessage, and device integrations), HEARTBEAT.md (periodic health ownership)"
qmd context add qmd://skills/ "OpenClaw skill definitions (SKILL.md files) with full API details, CLI commands, env vars, and troubleshooting for: grocery-reorder (Star Market), gws-gmail/calendar/drive (Google Workspace), cielo-ac (smart AC), mysa-thermostat (baseboard heaters), hue-lights, nest-thermostat (+ DASHBOARD.md), roomba, spotify-speakers, presence detection, restaurant-book (canonical dual-provider dinner coordinator), restaurant-snipe (bounded monitoring), attended opentable/resy booking, sag (TTS), peekaboo (cameras), samsung-tv, 1password, bluetooth, shortcuts, applescript, web-search, summarize, google-speakers, echonest, amazon-shopping, places, cabin/crosstown routines"
