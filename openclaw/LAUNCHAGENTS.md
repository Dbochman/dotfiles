...
Linux `tflite-runtime` declaration has no compatible wheel; the existing ONNX
...
backoff, because the reCAPTCHA iframe is present even before submission and is
not by itself evidence of a solved or blocked challenge. It never solves a
challenge. `cielo-reauth --attended start` owns the capture-first visible path,
and `cielo-reauth finish` requires and immediately proves a newly captured
refresh token before cleaning up its exact instance.
...
- **Gateway wrapper**: Uses the cache-only secrets pattern (`~/.openclaw/.secrets-cache`), no `op read` at startup (hangs under launchd), and is the final authority for the gateway process `PATH`. Its PATH starts with `~/.openclaw/bin` and keeps the explicit Node 22 keg path, so deployed skill wrappers pass `requires.bins` checks under both generated and recovery launch contracts. `dotfiles-pull.command` compares it with an owner-only last-activated hash, restarts changed or not-yet-activated content once, and records the new hash only after the replacement health check passes.
...
- **OpenAI authentication**: `ai.openclaw.oauth-refresh` manages Anthropic only. OpenClaw `2026.7.1-2` uses the canonical `openai` provider and stores its usable OAuth/token profiles in `~/.openclaw/agents/main/agent/openclaw-agent.sqlite`; the configured default is `openai/gpt-6-astra` with `agents.defaults.thinkingDefault` set to `medium`. Repair an invalidated profile with `openclaw models auth login --provider openai`, then confirm the intended order with `openclaw models status --json` and `openclaw models auth order`. Source `~/.openclaw/.secrets-cache` and include Homebrew Node in `PATH` before running these commands over SSH.
- **Astra model registration**: OpenClaw `2026.7.1-2` needs the explicit `gpt-6-astra` entry in `models.providers.openai.models` as well as the `agents.defaults.models` Codex runtime binding. On 2026-09-22, the catalog listed Astra but live turns rejected it until that definition was added. Its context metadata comes from the live catalog (`contextWindow: 872000`, `contextTokens: 272000`). A fresh-session test without model or thinking overrides then selected Astra with no fallback. Existing session or job overrides can still take precedence over the defaults.
- **Codex CLI upgrades**: The gateway invokes `/opt/homebrew/bin/codex` through a shared app-server client. If `npm install -g @openai/codex@latest` changes the installed version, restart `ai.openclaw.gateway` before the next model turn, wait for `/health`, and run the OpenAI verification below. Checking `codex --version` or the model catalog is insufficient because an already-running app-server can retain the replaced executable. This caused the 2026-07-09 fallback incident: Codex `0.144.0` was present on disk while the gateway still used shared app-server `0.142.5`.
- **OpenAI verification**: `openclaw models status --json` checks profile structure and routing but does not prove the provider still accepts the token. Require a live no-delivery smoke request and verify `agentMeta.provider` is `openai`, `agentMeta.model` is `gpt-6-astra`, and `executionTrace.fallbackUsed` is `false`:

  ```bash
  ssh dylans-mac-mini 'set -a; source ~/.openclaw/.secrets-cache; set +a; \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node@22/bin:$PATH \
    openclaw agent --agent main --session-key "agent:main:primary-auth-smoke-$(date +%s)" \
      --timeout 120 --json \
      --message "Reply with exactly AUTH_OK and nothing else."'
  ```
- **Cielo verification**: require `cielo-auth.py check` to report a protected refresh token and the `web-token-refresh-v1` contract after the first successful rotation. A zero LaunchAgent exit is necessary but not sufficient. Refresh `http://127.0.0.1:8558/api/status?refresh=true` and require the `cielo` object to have no `error`. If the chain is rejected, run the skill's `cielo-reauth --attended start` / `finish` sequence; access-only browser capture is explicitly non-durable.
