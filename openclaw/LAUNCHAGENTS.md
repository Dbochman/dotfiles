...
`presence-cabin-enroll` is an attended operator helper, not a LaunchAgent. It
has no schedule and does not restart the gateway. Its candidate config remains
under the private enrollment directory during shadow verification; promotion
to `~/.openclaw/presence-devices.json` is the explicit final activation
boundary because that path enables deployment of the strict Cabin scanner.
...
Both presence scanners require a site-scoped, owner-only mode-`0600`
`~/.openclaw/presence-devices.json` on their own host. Cabin binds Dylan and
Julia to exact Starlink captive client IDs and requires a live DHCP lease plus
at most five minutes of idle time; Crosstown binds them to exact site-private
MAC addresses with fresh inbound ARP reachability. Neither scanner has a name,
hostname, generic-iPhone, or IP fallback, and missing or malformed bindings
fail closed. Cabin ID provisioning remains an attended on-site prerequisite;
deployment must preserve the prior scanner until that protected config has
been created and validated without printing its values.
