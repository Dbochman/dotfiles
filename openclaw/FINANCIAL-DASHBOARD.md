...
The notifier reads Dylan's chat ID either from the one scoped environment value or from exactly one numeric `DYLAN_CHAT_ID` assignment read through a single verified owner-only secrets-cache file descriptor; it never executes or sources cache contents. It sends one fixed-method JSON-RPC request to `/opt/homebrew/bin/imsg rpc`, requires the already-supervised IMCore bridge transport, bounds stdin/stdout/stderr and the complete process group, and accepts only the matching JSON-RPC success receipt. This avoids the public AppleScript send path, whose automation call can hang without producing a local Messages row. A confirmed delivery is durably marked `sent` before cleanup, so delete/fsync failure never resends it; failed delivery returns to `pending` with exponential backoff capped at six hours. Invalid and crash-orphaned queue entries move to the private sibling quarantine without blocking valid UUID alerts. Safe start/completion status and counts live in `weekly-scrape-alert-notifier-status.json`. The unavoidable crash or ambiguous-timeout window between external send and the durable sent mark remains at-least-once and can rarely duplicate because `imsg` has no idempotency key. Neither notifier retry nor canary execution can run a scraper or import. Run `--canary` only during an attended delivery test after bridge readiness is confirmed.
...
1. The weekly preflight starts or reuses the existing dedicated headless
   `finance` profile without navigating or reading credentials. A failed
   preflight is reported even if cookie replay later succeeds.
2. The normal scrape first replays the protected cookie store and then uses raw
   CDP if the live Pinchtab tab remains authenticated.
3. When that fails, the helper runs `--verify-auth`. It may invoke
   `--boa-re-auth` only for the exact `not_authenticated` status, with
   credentials supplied only to that child process.
4. `--boa-re-auth` makes one raw-CDP submission and returns without retrying.
