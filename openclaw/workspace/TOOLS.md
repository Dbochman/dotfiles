...
Production source sync is deliberately separate from that cron: `ai.openclaw.finance-refresh` runs daily at 06:15 local time, invokes cache-only Plaid, crypto, and home-equity wrappers in that order, and never invokes `op`. Home equity contacts RentCast only for weekly-due properties and keeps aggregate-only status at `~/.openclaw/financial-dashboard/property-value-sync-status.json`. Combined status remains at `~/.openclaw/finance-refresh/status.json`; `not running` is normal between scheduled executions. The canonical Forecast financial source is `http://127.0.0.1:8585/api/forecast-baseline`, while live property provenance arrives through `8585 /api/mortgage/summary`.
