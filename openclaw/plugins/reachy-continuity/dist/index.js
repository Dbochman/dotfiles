...
export function remembersExplicitly(prompt) {
    ...
    if (negated.some((pattern) => pattern.test(text)))
        return false;
...
function normalizeDeliveredText(content) {
...
const plugin = definePluginEntry({
    ...
    register(api) {
        ...
        const queueSummary = (pending, assistant, runId) => {
        ...
        const finishTurn = (runId) => {
            const pending = pendingTurns.get(runId);
            if (!pending?.ended || !pending.succeeded)
                return;
        ...
        api.on("message_sent", (event, ctx) => {
            if (!event.success || event.to !== config.imessageTarget)
                return;
            const sessionKey = event.sessionKey ?? ctx.sessionKey;
            if (sourceForSession(config, sessionKey) !== "imessage")
                return;
            ...
            const pending = pendingTurns.get(runId);
            if (!pending || pending.source !== "imessage")
                return;
            const delivered = normalizeDeliveredText(event.content);
            if (!delivered)
                return;
            if (!pending.deliveredAssistant.includes(delivered))
                pending.deliveredAssistant.push(delivered);
        ...
        api.on("agent_end", (event, ctx) => {
            ...
            if (source === "imessage" && pendingTurns.has(runId)) {
                const cleanup = setTimeout(() => {
                    if (pendingTurns.get(runId) !== pending)
                        return;
                    pendingTurns.delete(runId);
