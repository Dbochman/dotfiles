...
5. The candidate config stays outside the production path throughout the soak.
   Merely creating `~/.openclaw/presence-devices.json` satisfies the existing
   deployment guard and could allow a routine pull to install the strict
   scanner, so promotion is the final activation boundary.
...
exact production schema at mode `0600` as the final capability-changing step.
...
8. Restore the receiver. After another correct evaluation, restore vacancy
   actions.
9. Restore the Nest activity reviewer last, only after presence remains correct
   and enrollment-window camera triggers are too old to review. Verify no
   delayed message or image analysis occurs.
10. Run `cleanup` to remove the redundant sensitive session and retain only the
    production binding plus safe report.
...
2. Move—not print or discard—the new production config into the protected
   rollback bundle.
