...
description: Temporarily pause, check, or restore the Mac Mini's browser-based BoA and Cielo LaunchAgents. Use when Dylan or Julia asks to prevent authentication windows, browser automation, or pop-ups while watching a show, movie, or other full-screen media on the Mac Mini. Also use when asked whether the viewing snooze is active or to resume automation afterward. NOT for stopping dashboards, OpenClaw, media playback, or unrelated background services.
...
The CLI records the original loaded set in
`~/.openclaw/state/mac-mini-automation-snooze.tsv`. Repeated `pause` calls preserve that
original set. A failed restore retains the state file and keeps the failed job disabled.

## Verification

After `pause`, require `snooze: active` and `unloaded, disabled` for every managed job.
