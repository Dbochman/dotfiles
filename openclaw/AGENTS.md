...
- Preserve the daily dependency order: cache-only Plaid source sync at 07:15 local time, then Forecast ledger capture at 07:35. Do not move or add a capture job that can run before its source refresh without an explicit recovery design.
